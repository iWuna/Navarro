...
/obj/item/weapon/card/id/syndicate/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	var/data[0]
	var/entries[0]
	entries[++entries.len] = list("name" = "Возраст", 				"value" = age)
	...
	entries[++entries.len] = list("name" = "Назначение",		"value" = assignment)
	if(GLOB.using_map.flags & MAP_HAS_BRANCH)
		entries[++entries.len] = list("name" = "Подразделение",		"value" = military_branch ? military_branch.name : "N/A")
	if(military_branch && (GLOB.using_map.flags & MAP_HAS_RANK))
		entries[++entries.len] = list("name" = "Звание",			"value" = military_rank ? military_rank.name : "N/A")
	entries[++entries.len] = list("name" = "Группа крови",		"value" = blood_type)
	...
	entries[++entries.len] = list("name" = "Имя", 				"value" = registered_name)
	entries[++entries.len] = list("name" = "Фото", 			"value" = "Update")
	entries[++entries.len] = list("name" = "Пол", 				"value" = sex)
