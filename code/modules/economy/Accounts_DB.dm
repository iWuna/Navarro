...
/obj/machinery/computer/account_database
	name = "база данных денежных средств"
	...
	proc/accounting_letterhead(report_name)
		return {"
			...
			<center><small><i>[station_name()] выписка по аккаунту</i></small></center>
			<hr>
			<u>Сгенерирован:</u> [held_card.registered_name], [held_card.assignment]<br>
		"}
...
/obj/machinery/computer/account_database/ui_interact(mob/user, ui_key="main", var/datum/nanoui/ui = null, var/force_open = 1)
	...
	for(var/i=1, i<=all_money_accounts.len, i++)
		...
		accounts.Add(list(list(\
			...
			"suspended"=D.suspended ? "ЗАМОРОЖЕН" : "",\
			"account_index"=i)))
	...
	if(href_list["choice"])
		switch(href_list["choice"])
			...
			if("finalise_create_account")
				...
				var/datum/money_account/new_account = create_account("Личный аккаунт [account_name]", account_name, starting_funds, ACCOUNT_TYPE_PERSONAL, src)
				if(starting_funds > 0)
					//subtract the money
					station_account.money -= starting_funds

					//create a transaction log entry
					new_account.deposit(starting_funds, "Активация нового аккаунта", machine_id)
			...
			if("print")
				...
				if (detailed_account_view)
					P.SetName("выписка по аккаунту #[detailed_account_view.account_number]")
					var/title = "Выписка по аккаунту #[detailed_account_view.account_number]"
					text = {"
						[accounting_letterhead(title)]
						<u>Держатель:</u> [detailed_account_view.owner_name]<br>
						<u>Баланс:</u> [GLOB.using_map.local_currency_name_short][detailed_account_view.money]<br>
						<u>Статус:</u> [detailed_account_view.suspended ? "Заморожен" : "Активен"]<br>
						<u>Транзакции:</u> ([detailed_account_view.transaction_log.len])<br>
						<table>
							<thead>
								<tr>
									<td>Временная подпись</td>
									<td>Цель</td>
									<td>Причина</td>
									<td>Значение</td>
									<td>Терминал</td>
					...
					for(var/i=1, i<=all_money_accounts.len, i++)
						...
						text += {"
								<tr>
									...
									<td>[D.suspended ? "Заморожен" : "Активен"]</td>
								</tr>
						"}
