...
/datum/event/solar_storm/announce()
	command_announcement.Announce("A solar storm has been detected approaching the [location_name()]. Please halt all EVA activites immediately and return inside.", "Сенсоры [location_name()]", zlevels = affecting_z)
...
/datum/event/solar_storm/start()
	command_announcement.Announce("The solar storm has reached the [location_name()]. Please refain from EVA and remain inside until it has passed.", "Сенсоры [location_name()]", zlevels = affecting_z)
