...
/datum/category_item/player_setup_item/occupation/content(mob/user, limit = 16, list/splitJobs, splitLimit = 1)
	...
	. += "<tt><center>"
	. += "<font size=3><b>Выберите и настройте профессию. Недоступные профессии пречёркнуты.</b></font>"
	. += "<br>"
		...
		if(!pref.hiding_maps[job_map])
			...
			for(var/datum/job/job in map_job_list)
				...
				if(job.total_positions == 0 && job.spawn_positions == 0)
					...
					bad_message = "<b>\[ЗАПРЕЩЕНО ДЛЯ ДАННОЙ РАСЫ]</b>"
				else if(!S.check_background(job, user.client.prefs))
					bad_message = "<b>\[ЗАПРЕЩЕНО ИЗ-ЗА БЭКА ПЕРСОНАЖА]</b>"
				...
				if(pref.points_by_job[job] && (!job.available_by_default || current_level != JOB_LEVEL_NEVER))
					skill_link = "<a class = 'Points' href='?src=\ref[src];set_skills=[title]'>Назначить скиллы</a>"
				else
					skill_link = "<a href='?src=\ref[src];set_skills=[title]'>Посмотреть скиллы</a>"
				...
				if(title == GLOB.using_map.default_assistant_title)//Assistant is special
					...
					. += "<font color = '#cccccc'>Не доступно на старте.</font>"
				else
					var/level_link
	...
	switch(pref.alternate_option)
		if(GET_RANDOM_JOB)
			. += "<u><a href='?src=\ref[src];job_alternative=1'>Получить случайную профессию если роль недоступна</a></u>"
		if(BE_ASSISTANT)
			. += "<u><a href='?src=\ref[src];job_alternative=1'>Быть ассистентом, если роль недоступна</a></u>"
		if(RETURN_TO_LOBBY)
			. += "<u><a href='?src=\ref[src];job_alternative=1'>Вернуться в лобби если роль недоступна</a></u>"
...
/datum/category_item/player_setup_item/occupation/OnTopic(href, href_list, user)
	if(href_list["reset_jobs"])
		...
		if(istype(job))
			...
			var/choice = input(user, "Выберите ваше звание.", CHARACTER_PREFERENCE_INPUT_TITLE) as null|anything in options
