...
/datum/admins/proc/startnow()
	set category = "Server"
	set desc="Начать раунд вот прям щас"
	set name="Начать сейчас"
	if(GAME_STATE < RUNLEVEL_LOBBY)
		to_chat(usr, "<span class='bigdanger'>Игра не может начаться, сервер не закончил инициализацию.</span>")
		SSticker.start_ASAP = !SSticker.start_ASAP
		if(SSticker.start_ASAP)
			to_chat(usr, "<span class='bigwarning'>Игра начнётся как можно скорее.</span>")
		else
			to_chat(usr, "<span class='bigwarning'>The game will begin as normal.</span>")
		return 0
	if(SSticker.start_now())
		log_admin("[usr.key] has started the game.")
		message_admins("<font color='blue'>[usr.key] начинает игру.</font>")
		...
		to_chat(usr, "<span class='bigwarning'>ОШИБКА: Начать сейчас: Игра уже началась.</span>")
		return 0

/datum/admins/proc/toggleenter()
	set category = "Server"
	set desc="Люди не смогут заходить"
	set name="Запретить вход"
