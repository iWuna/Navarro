...
/obj/screen/food/Click(var/location, var/control, var/params)
	if(istype(usr) && usr.nutrition_icon == src)
		switch(icon_state)
			if("nutrition0")
				to_chat(usr, SPAN_WARNING("Вы сыты."))
			if("nutrition1")
				to_chat(usr, SPAN_NOTICE("Вы не голодны."))
			if("nutrition2")
				to_chat(usr, SPAN_NOTICE("Вы немного голодны."))
			if("nutrition3")
				to_chat(usr, SPAN_WARNING("Вы голодны."))
			if("nutrition4")
				to_chat(usr, SPAN_DANGER("Вы голодаете!"))

/obj/screen/drink/Click(var/location, var/control, var/params)
	if(istype(usr) && usr.hydration_icon == src)
		switch(icon_state)
			if("hydration0")
				to_chat(usr, SPAN_WARNING("Вы перепили."))
			if("hydration1")
				to_chat(usr, SPAN_NOTICE("Вы не хотите пить."))
			if("hydration2")
				to_chat(usr, SPAN_NOTICE("Вы хотите пить."))
			if("hydration3")
				to_chat(usr, SPAN_WARNING("Ваше горло пересохло."))
			if("hydration4")
				to_chat(usr, SPAN_DANGER("Вы чувствуете изнеможение!"))
